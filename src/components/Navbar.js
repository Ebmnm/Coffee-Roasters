    ...
    return (
        <div className="nav-container">
           ...
           <div className={ toggleNav ? "show-links" : "hide-links" }>
               <Link onClick={() => setToggleNav(!toggleNav)}  className="nav-link" to="/Coffee-Roasters">HOME</Link>
               <Link onClick={() => setToggleNav(!toggleNav)}  className="nav-link" to="/plan">CREATE YOUR PLAN</Link>
           </div>
        </div>
    )
}
